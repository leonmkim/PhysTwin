...
parser.add_argument(
    "--keep-tmp-frames",
    action="store_true",
    help="Keep transient JPEG window under /tmp (debug only).",
)
...
USE_CONVERTED_SESSION = args.io_backend == "converted_session"

if not USE_CONVERTED_SESSION:
...
def list_jpeg_frame_names(frame_dir: str | Path) -> list[str]:
    frame_names = [
        p
...
def materialize_legacy_mp4_frames() -> str:
    ...
    return SOURCE_VIDEO_FRAME_DIR


def run_segmentation_pipeline(source_video_frame_dir: str | Path) -> None:
    frame_names = list_jpeg_frame_names(source_video_frame_dir)
...
if USE_CONVERTED_SESSION:
    ...
    with transient_jpeg_window(
        io_backend,
        camera_idx,
        max_frames=args.max_frames,
        stride=args.stride or 1,
        keep=args.keep_tmp_frames,
    ) as (source_video_frame_dir, frame_count):
        print(f"Converted-session JPEG window: {source_video_frame_dir} ({frame_count} frames)")
        run_segmentation_pipeline(source_video_frame_dir)
else:
    source_video_frame_dir = materialize_legacy_mp4_frames()
    run_segmentation_pipeline(source_video_frame_dir)
